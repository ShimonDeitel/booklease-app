...
final class Store: ObservableObject {
    @Published private(set) var items: [BookleaseItem] = []
    ...
    init() {
        let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Booklease", isDirectory: true)
    ...
    func add(title: String, note: String = "", value: Double = 0, date: Date = Date()) -> Bool {
        ...
        let item = BookleaseItem(title: title, note: note, value: value, date: date)
        items.insert(item, at: 0)
        save()
        return true
    }

    func update(_ item: BookleaseItem) {
    ...
    func delete(_ item: BookleaseItem) {
        items.removeAll { $0.id == item.id }
        save()
    }

    func toggleDone(_ item: BookleaseItem) {
        guard let idx = items.firstIndex(where: { $0.id == item.id }) else { return }
    ...
    private func load() {
        if let data = try? Data(contentsOf: fileURL),
           let decoded = try? JSONDecoder().decode([BookleaseItem].self, from: data) {
            items = decoded
        } else {
    ...
    static var seedData: [BookleaseItem] {
        let now = Date()
        let cal = Calendar.current
        return (0..<6).map { i in
            BookleaseItem(
                title: "Sample Textbook \(i + 1)",
